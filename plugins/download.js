    ...
    for (let item of data.BK9) {
      if (item.type === "video" && item.url) {
        await conn.sendMessage(from, {
          video: { url: item.url },
          mimetype: "video/mp4",
          caption: "📥 *Instagram Video Downloaded YOUSAF-BALOCH-MD*"
        }, { quoted: m });
        break; // Stop once we send the first video
      } else if (item.type === "image" && item.url) {
        await conn.sendMessage(from, {
          image: { url: item.url },
          caption: "📥 *Instagram Image Downloaded YOUSAF-BALOCH-MD*"
        }, { quoted: m });
  ...
  if (isReplyToBotMessage) {
    ...
    if (userResponse === '1') {
      await conn.sendMessage(chatID, {
        ...
        'caption': "*Downloaded YOUSAF-BALOCH-MD*"
    ...
    switch (receivedText) {
      ...
      case "4":
        await conn.sendMessage(senderID, {
          ...
          caption: "📥 *Audio Downloaded YOUSAF-BALOCH-MD*"
        }, { quoted: receivedMsg });
        break;
  ...
  try {
    ...
    if (downloadUrl) {
      ...
      await conn.sendMessage(from, {
        ...
        caption: "*© Powered By Mʀ-YOUSAF BALOCH*"
      }, { quoted: m });
